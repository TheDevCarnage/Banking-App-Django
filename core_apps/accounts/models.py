...
class BankAccount(TimeStampedModel):
    ...
    account_balance = models.DecimalField(
        _("Account Balance"), decimal_places=3, max_digits=10, default=0.00
    )
    currency = models.CharField(
        _("Currency"),
        max_length=20,
        choices=AccountCurrency.choices,
        default=AccountCurrency.DOLLAR,
    )
...
class Transaction(TimeStampedModel):
    ...
    amount = models.DecimalField(
        _("Amount"), decimal_places=2, max_digits=10, default=0.00
    )
    description = models.CharField(
        _("Description"), max_length=500, null=True, blank=True
    )
    receiver = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="received_transactions"
    )
